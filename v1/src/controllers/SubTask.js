const { insert, list, modify, remove } = require("../services/SubTask")
const TaskCreate = require("../services/Tasks")
...
const create = (req, res) => {
    ...
    TaskCreate.insert(req.body).then((response) => {
        res.status(httpStatus.CREATED).send(response);
    }).catch((e) => {
        res.status(httpStatus.INTERNAL_SERVER_ERROR).send(e)
    })
    insert(parseInt(req.params.task_id)).then((response) => {
        res.status(httpStatus.CREATED).send(response);
    }).catch((e) => {
        res.status(httpStatus.INTERNAL_SERVER_ERROR).send(e)
    })
